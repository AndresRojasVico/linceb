...
new class extends Component {
    ...
    {
        // IDs de los proyectos favoritos del usuario autenticado
        $favoritesIds = UserProjectFavorite::where('user_id', Auth::id())
            ->pluck('project_id');

        $query = \App\Models\Project::whereIn('id', $favoritesIds);
...
<div>
    ...
    @if($this->projects->isEmpty())
        <div class="flex flex-col items-center justify-center py-16 text-neutral-400">
            <flux:icon name="heart" class="size-10 mb-3 opacity-40" />
            <p class="text-sm">No tienes proyectos favoritos todavía.</p>
        </div>
    @else
